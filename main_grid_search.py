...
for counter_file,data_file in enumerate(data_files):
	...
	for iteration in range(10):
		...
		if FLAGS.use_nystroem:
		    Q, subgraphs, labels,shapes = compute_nystroem(data_file, use_node_labels, FLAGS.embedding_dim,FLAGS.community_detection, kernels)
		else:
		...
		for train_index, test_index in kf.split(x):
			...
			for batch_size in batch_sizes:
				for num_epochs in num_epochs_ar:
					...
					clf=cnn_classifier(sequence_length=x_train.shape[1],num_classes=y_train.shape[1],vocab_size=len(vocab_processor.vocabulary_),num_kernels=num_kernels,Q=Q,FLAGS=FLAGS)
		...
		std = np.std(accs)
		results_file.write("Mean Accuracy: "+str(mean_acc)+", Std:"+str(std)+", Mean time:"+str(mean_time)+"\n\n")
		results_file.write("------------------------------\n\n")
